	...
	private function parseMtl(data:String):Void {
		...
		for (i in 0...materialDefinitions.length) {
				...
				if (lines[j].substring(0, 1) != "#" && (j == 0 || lines[j] != "")) {
					...
					if (j == 0) {
						...
						switch (trunk[0]) {
							case "Ka":
								if (trunk[1] != ""
									&& !Math.isNaN(Std.parseFloat(trunk[1]))
									&& trunk[2] != ""
									&& !Math.isNaN(Std.parseFloat(trunk[2]))
									&& trunk[3] != ""
									&& !Math.isNaN(Std.parseFloat(trunk[3])))
			...
			if (mapkd != "") {
				...
				if (alpha == 0)
					Debug.trace("Warning: an alpha value of 0 was found in mtl color tag (Tr or d) ref:"
						+ _lastMtlID
						+ ", mesh(es) using it will be invisible!");
