...
class DAEParser extends ParserBase {
	...
	override private function proceedParsing():Bool {
		...
		switch (_parseState) {
			...
			case DAEParserState.PARSE_MATERIALS:
				_libMaterials = parseLibrary(_fastDoc.hasNode.library_materials ? _fastDoc.node.resolve("library_materials")
					.nodes.resolve("material") : empty, DAEMaterial);
				_libEffects = parseLibrary(_fastDoc.hasNode.library_effects ? _fastDoc.node.resolve("library_effects").nodes.resolve("effect") : empty,
					DAEEffect);
				setupMaterials();
				_parseState = DAEParserState.PARSE_GEOMETRIES;

			case DAEParserState.PARSE_GEOMETRIES:
				_libGeometries = parseLibrary(_fastDoc.hasNode.library_geometries ? _fastDoc.node.resolve("library_geometries")
					.nodes.resolve("geometry") : empty, DAEGeometry);
				_geometries = translateGeometries();
				_parseState = DAEParserState.PARSE_CONTROLLERS;

			case DAEParserState.PARSE_CONTROLLERS:
				_libControllers = parseLibrary(_fastDoc.hasNode.library_controllers ? _fastDoc.node.resolve("library_controllers")
					.nodes.resolve("controller") : empty, DAEController);
	...
	private function translatePrimitive(mesh:DAEMesh, primitive:DAEPrimitive, reverseTriangles:Bool = true, autoDeriveVertexNormals:Bool = true,
			autoDeriveVertexTangents:Bool = true):CompactSubGeometry {
...
class DAEColor {
	...
	private function set_rgb(val:UInt):UInt {
		r = (val >> 16) / 255.0;
		g = (val >> 8) / 255.0;
		b = val / 255.0;
		return val;
