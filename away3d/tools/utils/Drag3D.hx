...
class Drag3D {
	...
	private function intersect(?x:Float, ?y:Float):Void {
		var pMouse:Vector3D = ((x == null && y == null)) ? _view.unproject(_view.mouseX, _view.mouseY, 1,
			Matrix3DUtils.CALCULATION_VECTOR3D) : _view.unproject(x, y, 1, Matrix3DUtils.CALCULATION_VECTOR3D);
