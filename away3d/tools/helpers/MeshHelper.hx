...
class MeshHelper {
	...
	public static function boundingRadius(mesh:Mesh):Float {
		...
		try {
			...
			radius = Math.max((Bounds.maxX - Bounds.minX) * mesh.scaleX,
				Math.max((Bounds.maxY - Bounds.minY) * mesh.scaleY, (Bounds.maxZ - Bounds.minZ) * mesh.scaleZ));
		}
	...
	public static function boundingRadiusContainer(container:ObjectContainer3D):Float {
		...
		var radius:Float = Math.max((Bounds.maxX - Bounds.minX) * container.scaleX,
			Math.max((Bounds.maxY - Bounds.minY) * container.scaleY, (Bounds.maxZ - Bounds.minZ) * container.scaleZ));
		return radius * .5;
	}
	...
	public static function splitMesh(mesh:Mesh, disposeSource:Bool = false):Vector<Mesh> {
		...
		if (isOfType(geometries[0], CompactSubGeometry))
