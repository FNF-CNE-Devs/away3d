...
class LinearExtrude extends Mesh {
	...
	private function buildThicknessPoints(prop1:String, prop2:String):Array<FourPoints> {
		...
		for (i in 0..._aVectors.length - 1) {
			...
			if (Reflect.getProperty(_aVectors[i + 1], prop2)
			...
			if (Reflect.getProperty(_aVectors[i], prop1)
	...
	private function defineAnchors(base:Vector3D, baseEnd:Vector3D, prop1:String, prop2:String):FourPoints {
		var angle:Float = (Math.atan2(Reflect.getProperty(base, prop2) - Reflect.getProperty(baseEnd, prop2),
			Reflect.getProperty(base, prop1) - Reflect.getProperty(baseEnd, prop1)) * 180) / Math.PI;
		angle -= 270;
