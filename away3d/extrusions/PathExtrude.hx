...
class PathExtrude extends Mesh {
	 ...
	 */
	public function new(material:MaterialBase = null, path:IPath = null, profile:Vector<Vector3D> = null, subdivision:Int = 2, coverAll:Bool = true,
			coverSegment:Bool = false, alignToPath:Bool = true, centerMesh:Bool = false, mapFit:Bool = false, flip:Bool = false, closePath:Bool = false,
			materials:Vector<MaterialBase> = null, scales:Vector<Vector3D> = null, smoothScale:Bool = true, rotations:Vector<Vector3D> = null,
			smoothSurface:Bool = true, distribute:Bool = false, distributeU:Bool = true, keepExtremes:Bool = false) {
	...
	private function set_profile(val:Vector<Vector3D>):Vector<Vector3D> {
		...
		if (_profile != null)
			_isClosedProfile = (_profile[0].x == _profile[_profile.length - 1].x
				&& _profile[0].y == _profile[_profile.length - 1].y
				&& _profile[0].z == _profile[_profile.length - 1].z);
	...
	private function set_smoothSurface(val:Bool):Bool {
		...
		_geomDirty = true;
		return _smoothSurface = val;
	}

	/**
	 * Defines if the texture(s) should applied per segment. Default false.
	 */
	private inline function set_coverSegment(b:Bool):Bool {
		return _coverSegment = b;
	}
	...
	private inline function set_keepExtremes(b:Bool):Bool {
		return _keepExtremes = b;
	}
