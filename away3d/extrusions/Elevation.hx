...
class Elevation extends Mesh {
	 ...
	 */
	public function new(material:MaterialBase, heightMap:BitmapData, width:Float = 1000, height:Float = 100, depth:Float = 1000, segmentsW:Int = 30,
			segmentsH:Int = 30, maxElevation:Int = 255, minElevation:Int = 0, smoothMap:Bool = false) {
	...
	public function getHeightAt(x:Float, z:Float):Float {
		...
		return
			((col > _maxElevation)) ? (_maxElevation / 0xff) * _height : (((col < _minElevation)) ? (_minElevation / 0xff) * _height : (col / 0xff) * _height);
	}
