...
class ParticleFollowState extends ParticleStateBase {
	...
	override public function setRenderState(stage3DProxy:Stage3DProxy, renderable:IRenderable, animationSubGeometry:AnimationSubGeometry,
		...
		if (_particleFollowNode._usesPosition && _particleFollowNode._usesRotation) {
			...
			animationSubGeometry.activateVertexBuffer(animationRegisterCache.getRegisterIndex(_animationNode, ParticleFollowNode.FOLLOW_POSITION_INDEX),
				_particleFollowNode.dataOffset, stage3DProxy, Context3DVertexBufferFormat.FLOAT_3);
			animationSubGeometry.activateVertexBuffer(animationRegisterCache.getRegisterIndex(_animationNode, ParticleFollowNode.FOLLOW_ROTATION_INDEX),
				_particleFollowNode.dataOffset + 3, stage3DProxy, Context3DVertexBufferFormat.FLOAT_3);
	...
	private function processPosition(currentTime:Float, deltaTime:Float, animationSubGeometry:AnimationSubGeometry):Void {
		...
		for (i in 0...len) {
			...
			if (t - deltaTime <= 0) {
				...
				if (vertexData[inc] != interpolatedPos.x
					|| vertexData[inc + 1] != interpolatedPos.y
					|| vertexData[inc + 2] != interpolatedPos.z) {
					changed = true;
	...
	private function precessRotation(currentTime:Float, deltaTime:Float, animationSubGeometry:AnimationSubGeometry):Void {
		...
		for (i in 0...len) {
			...
			if (t - deltaTime <= 0) {
				...
				if (vertexData[inc] != interpolatedRotation.x
					|| vertexData[inc + 1] != interpolatedRotation.y
					|| vertexData[inc + 2] != interpolatedRotation.z) {
					changed = true;
					for (j in 0...data[i].numVertices) {
		...
		for (i in 0...len) {
			...
			if (t - deltaTime <= 0) {
				...
				if (vertexData[inc] != interpolatedPos.x
					|| vertexData[inc + 1] != interpolatedPos.y
					|| vertexData[inc + 2] != interpolatedPos.z
					|| vertexData[inc + 3] != interpolatedRotation.x
					|| vertexData[inc + 4] != interpolatedRotation.y
					|| vertexData[inc + 5] != interpolatedRotation.z) {
					changed = true;
					for (j in 0...data[i].numVertices) {
						vertexData[inc++] = interpolatedPos.x;
						vertexData[inc++] = interpolatedPos.y;
