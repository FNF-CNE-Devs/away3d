...
class SkinnedSubGeometry extends CompactSubGeometry {
	...
	public function activateJointIndexBuffer(index:Int, stage3DProxy:Stage3DProxy):Void {
		...
		if (_jointIndicesInvalid[contextIndex]) {
			_jointIndexBuffer[contextIndex].uploadFromVector(_numCondensedJoints > 0 ? cast _condensedJointIndexData : cast _jointIndexData, 0,
				Std.int(_jointIndexData.length / _jointsPerVertex));
