...
class Quaternion {
	...
	public function fromEulerAngles(ax:Float, ay:Float, az:Float):Void {
		var halfX:Float = ax * .5,
			halfY:Float = ay * .5,
			halfZ:Float = az * .5;
		var cosX:Float = Math.cos(halfX), sinX:Float = Math.sin(halfX);
		var cosY:Float = Math.cos(halfY), sinY:Float = Math.sin(halfY);
		var cosZ:Float = Math.cos(halfZ), sinZ:Float = Math.sin(halfZ);
