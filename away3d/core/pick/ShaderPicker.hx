...
class ShaderPicker implements IPicker {
	...
	private function getPreciseDetails(camera:Camera3D):Void {
		...
		var x:Float = _localHitPosition.x,
			y:Float = _localHitPosition.y,
			z:Float = _localHitPosition.z;
		var u:Float, v:Float;
		...
		while (i < len) {
			...
			// if within bounds
			if (!((x < x1 && x < x2 && x < x3) || (y < y1 && y < y2 && y < y3) || (z < z1 && z < z2 && z < z3) || (x > x1 && x > x2 && x > x3)
				|| (y > y1 && y > y2 && y > y3) || (z > z1 && z > z2 && z > z3))) {
				...
				if (s >= 0 && t >= 0 && (s + t) <= 1) {
					...
					nl = 1 / Math.sqrt(_localHitNormal.x * _localHitNormal.x + _localHitNormal.y * _localHitNormal.y +
						_localHitNormal.z * _localHitNormal.z); // normalize n
