		...
		for (i in 0..._numEntities) {
			...
			if (entity.pickingCollider != null) {
				...
				if ((bestCollisionVO == null || pickingCollisionVO.rayEntryDistance < bestCollisionVO.rayEntryDistance)
					&& entity.collidesBefore(shortestCollisionDistance, _findClosestCollision)) {
			...
			} else if (bestCollisionVO == null
				|| pickingCollisionVO.rayEntryDistance < bestCollisionVO.rayEntryDistance) { // A bounds collision with no triangle collider stops all checks.
