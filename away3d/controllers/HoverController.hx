...
class HoverController extends LookAtController {
	 ...
	 */
	public function new(targetObject:Entity = null, lookAtObject:ObjectContainer3D = null, panAngle:Float = 0, tiltAngle:Float = 90, distance:Float = 1000,
			minTiltAngle:Float = -90, maxTiltAngle:Float = 90, ?minPanAngle:Float = null, ?maxPanAngle:Float = null, ?steps:Int = 8, ?yFactor:Float = 2,
			?wrapPanAngle:Bool = false) {
	...
	override public function update(interpolate:Bool = true):Void {
		...
		_targetObject.x = _pos.x
			+ _distance * Math.sin(_currentPanAngle * MathConsts.DEGREES_TO_RADIANS) * Math.cos(_currentTiltAngle * MathConsts.DEGREES_TO_RADIANS);
		_targetObject.z = _pos.z
			+ _distance * Math.cos(_currentPanAngle * MathConsts.DEGREES_TO_RADIANS) * Math.cos(_currentTiltAngle * MathConsts.DEGREES_TO_RADIANS);
