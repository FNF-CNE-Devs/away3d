...
class TorusGeometry extends PrimitiveBase {
	...
	override private function buildGeometry(target:CompactSubGeometry):Void {
		...
		var x:Float,
			...
			ny:Float,
			nz:Float,
			revolutionAngleR:Float,
			revolutionAngleT:Float;
		...
		for (j in 0..._segmentsT + 1) {
			...
			for (i in 0..._segmentsR + 1) {
				...
				if (i == _segmentsR) {
					addVertex(x, _rawVertexData[startIndex + 1], _rawVertexData[startIndex + 2], nx, n1, n2, -((length != 0) ? ny / length : y / _radius), t1,
						t2);
				} else {
