...
class CubeGeometry extends PrimitiveBase {
	...
	override private function buildGeometry(target:CompactSubGeometry):Void {
		...
		var numVerts:Int = ((_segmentsW + 1) * (_segmentsH + 1) + (_segmentsW + 1) * (_segmentsD + 1) + (_segmentsH + 1) * (_segmentsD + 1)) * 2;
	...
	override private function buildUVs(target:CompactSubGeometry):Void {
		...
		var numUvs:Int = Std.int(((_segmentsW + 1) * (_segmentsH + 1) + (_segmentsW + 1) * (_segmentsD + 1) + (_segmentsH + 1) * (_segmentsD +
			1)) * 2 * stride);
