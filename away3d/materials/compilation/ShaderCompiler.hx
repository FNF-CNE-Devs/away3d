...
class ShaderCompiler {
	...
	private function compileProjectionCode():Void {
		var pos:String = _dependencyCounter.globalPosDependencies > 0
			|| _forceSeperateMVP ? _sharedRegisters.globalPositionVertex.toString() : _animationTargetRegisters[0];
		var code:String;
	...
	private function initLightData():Void {
		...
		_usingSpecularMethod = (_methodSetup._specularMethod != null && (usesLightsForSpecular() || usesProbesForSpecular()));
	}
	...
	private function compileMethods():Void {
		...
		if (_methodSetup._colorTransformMethod != null) {
			...
			_fragmentCode += _methodSetup._colorTransformMethod.getFragmentCode(_methodSetup._colorTransformMethodVO, _registerCache,
