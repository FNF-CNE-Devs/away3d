...
class SuperShaderCompiler extends ShaderCompiler {
	...
	override private function compileNormalCode():Void {
		...
		if (_methodSetup._normalMethod.hasOutput) {
			...
			_vertexCode += "m33 " + _sharedRegisters.normalVarying + ".xyz, " + _sharedRegisters.animatedNormal + ", " + normalMatrix[0] + "\n" + "mov "
				+ _sharedRegisters.normalVarying + ".w, " + _sharedRegisters.animatedNormal + ".w	\n";

			_fragmentCode += "nrm " + _sharedRegisters.normalFragment + ".xyz, " + _sharedRegisters.normalVarying + "\n" + "mov "
				+ _sharedRegisters.normalFragment + ".w, " + _sharedRegisters.normalVarying + ".w		\n";
	...
	private function compileTangentVertexCode(matrix:Vector<ShaderRegisterElement>):Void {
		...
		_vertexCode += "m33 " + _sharedRegisters.animatedNormal + ".xyz, " + _sharedRegisters.animatedNormal + ", " + matrix[0] + "\n" + "nrm "
			+ _sharedRegisters.animatedNormal + ".xyz, " + _sharedRegisters.animatedNormal + "\n";

		_vertexCode += "m33 " + _sharedRegisters.animatedTangent + ".xyz, " + _sharedRegisters.animatedTangent + ", " + matrix[0] + "\n" + "nrm "
			+ _sharedRegisters.animatedTangent + ".xyz, " + _sharedRegisters.animatedTangent + "\n";
		...
		_vertexCode += "mov " + _sharedRegisters.tangentVarying + ".x, " + _sharedRegisters.animatedTangent + ".x  \n" + "mov "
			+ _sharedRegisters.tangentVarying + ".z, " + _sharedRegisters.animatedNormal + ".x  \n" + "mov " + _sharedRegisters.tangentVarying + ".w, "
			+ _sharedRegisters.normalInput + ".w  \n" + "mov " + _sharedRegisters.bitangentVarying + ".x, " + _sharedRegisters.animatedTangent + ".y  \n"
			+ "mov " + _sharedRegisters.bitangentVarying + ".z, " + _sharedRegisters.animatedNormal + ".y  \n" + "mov " + _sharedRegisters.bitangentVarying
			+ ".w, " + _sharedRegisters.normalInput + ".w  \n" + "mov " + _sharedRegisters.normalVarying + ".x, " + _sharedRegisters.animatedTangent
			+ ".z  \n" + "mov " + _sharedRegisters.normalVarying + ".z, " + _sharedRegisters.animatedNormal + ".z  \n" + "mov "
			+ _sharedRegisters.normalVarying + ".w, " + _sharedRegisters.normalInput + ".w  \n" + "crs " + bitanTemp + ".xyz, "
			+ _sharedRegisters.animatedNormal + ", " + _sharedRegisters.animatedTangent + "\n" + "mov " + _sharedRegisters.tangentVarying + ".y, "
			+ bitanTemp + ".x    \n" + "mov " + _sharedRegisters.bitangentVarying + ".y, " + bitanTemp + ".y  \n" + "mov " + _sharedRegisters.normalVarying
			+ ".y, " + bitanTemp + ".z    \n";
	...
	private function compileTangentNormalMapFragmentCode():Void {
		...
		_fragmentCode += _methodSetup._normalMethod.getFragmentCode(_methodSetup._normalMethodVO, _registerCache, temp)
			...
			+ _sharedRegisters.normalFragment
			+ ".w,   "
			+ _sharedRegisters.normalVarying
			+ ".w			\n";
	...
	override private function compileViewDirCode():Void {
		...
		_vertexCode += "sub "
			+ _sharedRegisters.viewDirVarying
			+ ", "
			+ cameraPositionReg
			+ ", "
			+ _sharedRegisters.globalPositionVertex
			+ "\n";
	...
	override private function compileLightingCode():Void {
		...
		if (_alphaPremultiplied) {
			_fragmentCode += "add " + _sharedRegisters.shadedTarget + ".w, " + _sharedRegisters.shadedTarget + ".w, " + _sharedRegisters.commons + ".z\n"
				...
				+ _sharedRegisters.shadedTarget + ".w, " + _sharedRegisters.shadedTarget + ".w, " + _sharedRegisters.commons + ".z\n" + "sat "
				+ _sharedRegisters.shadedTarget + ".xyz, " + _sharedRegisters.shadedTarget + "\n";
		}
		...
		if (_usingSpecularMethod) {
			...
			_fragmentCode += _methodSetup._specularMethod.getFragmentPostLightingCode(_methodSetup._specularMethodVO, _registerCache,
				_sharedRegisters.shadedTarget);
	...
	private function compileDirectionalLightCode():Void {
		...
		for (i in 0..._numDirectionalLights) {
			...
			if (addDiff)
				_fragmentCode += _methodSetup._diffuseMethod.getFragmentCodePerLight(_methodSetup._diffuseMethodVO, lightDirReg, diffuseColorReg,
					_registerCache);
			if (addSpec)
				_fragmentCode += _methodSetup._specularMethod.getFragmentCodePerLight(_methodSetup._specularMethodVO, lightDirReg, specularColorReg,
					_registerCache);
		}
	}
	...
	private function compilePointLightCode():Void {
		...
		for (i in 0..._numPointLights) {
			...
			_fragmentCode += "sub "
				...
				+ lightPosReg
				+ ", "
				+ _sharedRegisters.globalPositionVarying
				+ "\n"
				+ // attenuate
				"dp3 "
				+ lightDirReg
