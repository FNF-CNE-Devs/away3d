...
class LightingShaderCompiler extends ShaderCompiler {
	...
	private function compileTangentSpaceNormalMapCode():Void {
		...
		_vertexCode += "crs "
			+ _sharedRegisters.bitangent
			+ ".xyz, "
			+ _sharedRegisters.animatedNormal
			+ ", "
			+ _sharedRegisters.animatedTangent
			+ "\n";
	...
	override private function compileViewDirCode():Void {
		...
		if (tangentSpace) {
			...
			_vertexCode += "sub " + temp + ", " + cameraPositionReg + ", " + _sharedRegisters.localPosition + "\n" + "m33 "
				+ _sharedRegisters.viewDirVarying + ".xyz, " + temp + ", " + _sharedRegisters.animatedTangent + "\n" + "mov "
				+ _sharedRegisters.viewDirVarying + ".w, " + _sharedRegisters.localPosition + ".w\n";
		} else {
			_vertexCode += "sub "
