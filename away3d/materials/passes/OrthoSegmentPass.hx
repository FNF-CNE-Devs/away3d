 ...
 */
class OrthoSegmentPass extends MaterialPassBase {
	private static var ONE_VECTOR:Vector<Float> = new Vector<Float>([1, 1, 1, 1]);
	private static var FRONT_VECTOR:Vector<Float> = new Vector<Float>([0, 0, 1, 1]);
	...
	private override function render(renderable:IRenderable, stage3DProxy:Stage3DProxy, camera:Camera3D, viewProjection:Matrix3D):Void {
		...
		var subSetCount:UInt = cast(renderable, SegmentSet).subSetCount;

		if (cast(renderable, SegmentSet).hasData) {
	...
	private override function activate(stage3DProxy:Stage3DProxy, camera:Camera3D):Void {
		...
		if (stage3DProxy.scissorRect != null)
			_constants[0] = _thickness * 0.5 * cast(camera.lens, OrthographicLens).projectionHeight / stage3DProxy.scissorRect.height;
		else
			_constants[0] = _thickness * 0.5 * cast(camera.lens, OrthographicLens).projectionHeight / stage3DProxy.height;
